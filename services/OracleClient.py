...
class OracleClient:
    ...
    def get_con(self) -> oracledb.Connection:
        try:
            print(f"oracle_user: {self.oracle_user}, oracle_pass: {'*' * len(self.oracle_pass) if self.oracle_pass else None}, oracle_host: {self.oracle_host}, oracle_port: {self.oracle_port}, oracle_service: {self.oracle_service}")
