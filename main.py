...
python ./charon.py -v `
    ...
    --source-environment devint `
    --source-namespace 1-inprs-automation `
    --target-system oracle `
    --target-environment dev02 `
    --target-namespace BRUDOW `
    --action upsert `
    --tables Case CaseComment InteractionSummary Account
    #--tables Case CaseComment InteractionSummary Account
"""
...
def prepare_child(
    ...
    """Resolves execution paths, formats virtual environments, and injects runtime logging bootstraps."""
...
def setup_logging(
    ...
    """Hooks into stdout streams and appends persistent runtime file-logging instances."""
...
def parse_config_file(config_path: str | Path = "", env: str = "") -> dict[str, str]:
    ...
    os.environ['ROOT_PATH'] = root_path.upper()  # Force internal path marker to caps
    default_loc = DEFAULT_CONFIG_FILE
    ...
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            v_upper = val.strip().strip('"').strip("'")
            raw[k_upper] = v_upper
            
    # Normalize environment variables and engine flags to uppercase for perfect interpolation
    env_upper = {k.upper(): v.upper() for k, v in os.environ.items()}
    lookup = {**env_upper, **raw, "ENV": env.upper()}
    ...
    def interpolate(val: str) -> str:
        ...
        while val != previous and loops < 10:
            ...
            def repl(m: re.Match) -> str:
                matched_name = m.group(1) or m.group(2)
                if not matched_name:
                    return m.group(0)
                
                # Force requested lookup token to uppercase
                name = matched_name.upper()
...
def parse_args(argv) -> argparse.Namespace:
    """Defines orchestrator runtime boundary requirements and isolates trailing execution arguments."""
    _env_help_msg = "Environment (dev01, mdmdev, sit01, etc...) NOTE: This is not for .env files, use --config for those."
...
def main():
    """Validates Python target versions, builds runtime dependencies, and tracks asynchronous child pipes."""
    ...
    try:
        ...
        config_vars = parse_config_file(args.config, env=args.env)
        cmd, child_env, child_cwd = prepare_child(args, config_vars)
        
        logger.info(
            f"Spawning child command: {cmd}\n"
            f"Verifying child executable exists: {os.path.exists(cmd[0])}\n"
            f"Child working directory: {child_cwd}"
        )
