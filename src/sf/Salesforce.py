...
from src.sf.SfModels import SKIP_NAMES, SKIP_SUFFIXES, HttpMethod as http, SalesforceRequestError
...
class Salesforce(DataSource):
    ...
    def _rest_query_records(self, soql: str) -> Iterator[dict[str, Any]]:
        resp: Any = self.client.request(http.get, "query/", params={"q": soql}).json()
    ...
    def describe_schema(self, namespace: str | None = None, environment: str | None = None) -> Schema:
        # resp: Any = self.client.request("GET", "sobjects/").json()
    ...
    def describe_table(self, table: Table) -> Table:
        resp: Any = self.client.request(http.get, f"sobjects/{table.name}/describe/").json()
    ...
    def mutate_table(self, table: Table, source_system: System | None = None) -> Table:
        try:
